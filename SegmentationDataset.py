...
# todo c
factory = ([255,0,255], 0)  # （像素值， 标签）
residential_building = ([0, 0, 255], 1)
road = ([255,255,0], 2)
water = ([0,255,255], 3)
...
class CustomSegmentation(BaseDataset):  # base_size 2048 crop_size 768
    ...
    def __getitem__(self, index):
        ...
        if self.target_transform is not None:
            mask = self.target_transform(mask)
        # todo img =[3, 1024, 1024]
        # todo mask = [1024, 1024, 4]
        return img, mask
...
# todo 调用顺序 loader->Segmentation->pairs
# todo 专门加载bdd的数据集
def get_bdd_loader(root=os.path.expanduser('data/citys/'), split="train", mode="train",  # 获取训练和验证用的dataloader
                     base_size=1024, crop_size=(1024, 1024),
                     batch_size=32, workers=4, pin=True):
    if mode == "train":
        input_transform = transforms.Compose([
    ...
    dataset = BddSegmentation(root=root, split=split, mode=mode,
                               transform=input_transform,
                               base_size=base_size, crop_size=crop_size, low=0.65, high=2, sample_std=40)
...
class BddSegmentation(BaseDataset):  # base_size 2048 crop_size 768
    # mode训练时候验证用testval, 测试验证集指标时候也用testval, val倍废弃
    def __init__(self, root=os.path.expanduser('../data/citys/'), split='train',
                 mode=None, transform=None, target_transform=None, NUM_CLASS=5, **kwargs):
        super(BddSegmentation, self).__init__(
            root, split, mode, transform, target_transform, **kwargs)
        # self.root = os.path.join(root, self.BASE_DIR)
        # self.images, self.mask_paths = get_city_pairs(self.root, self.split)
        self.images, self.mask_paths = get_bdd_pairs(self.root, self.split)
        assert (len(self.images) == len(self.mask_paths))
        if len(self.images) == 0:
            raise RuntimeError("Found 0 images in subfolders of: \
                " + self.root + "\n")
    ...
    def _mask_transform2(self, mask):
        target = self.mask_generate(np.array(mask).astype('int32'))
        return torch.from_numpy(target).long()
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            mask = self._mask_transform2(mask)
            ...
            mask = self._mask_transform2(mask)
        # todo 最终生生成的mask应该是图片的大小 [1024, 1024]
        # general resize, normalize and toTensor
        if self.transform is not None:
            img = self.transform(img)
        if self.target_transform is not None:
...
def get_bdd_pairs(folder, split='train'):
    def get_path_pairs(img_folder, mask_folder):
        img_paths = []
        mask_paths = []
        for root, directories, files in os.walk(img_folder):
