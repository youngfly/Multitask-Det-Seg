...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        print("\n")
        print("load the det train data ok")
    ...
    #                                                                  # 我电脑上(4,4)是最快的, 更大子进程会挂(现在图大了,怎么设都会挂, BUG)
        seg_valloader = SegmentationDataset.get_bdd_loader(root=segval_path, batch_size=1,
                                                         split="val", mode="testval",  # 旧版为val新版训练中验证也用testval模式
                                                         base_size=1024,   # 对cityscapes, 原图resize到(1024, 512)输入后双线性插值到原图尺寸计算精度
                                                         # crop_size=640,  # testval 时候cropsize不起作用
                                                         workers=4, pin=True)  # 验证batch_size和workers得配合, 都太大会导致子进程死亡, 单进程龟速加载数据
    ...
    # todo 采用
    seg_trainloader = SegmentationDataset.get_bdd_loader(root=segtrain_path,
                                                           split="train", mode="train",
                                                           base_size=1024,  # cropsize长边和检测同，短边砍半(针对cityscapes,bdd也行)
                                                           batch_size=batch_size,
                                                           workers=opt.workers, pin=True)
    print("\n")
    print("load the seg train data ok")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='/workspace/pretrained model/yolov5s.pt', help='initial weights path')
